...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func getComplicationDescriptors(handler: @escaping ([CLKComplicationDescriptor]) -> Void) {
        ...
        let bellyrubsIdentifier = "testComplication"
        let supportedFamilies = [CLKComplicationFamily.graphicCorner, .graphicCircular, .circularSmall]
        let bellyRubsDescriptor = CLKComplicationDescriptor(
            identifier: bellyrubsIdentifier,
            displayName: "Bellyrubs",
            supportedFamilies: supportedFamilies)
        
        // Call the handler with the currently supported complication descriptors
        handler([bellyRubsDescriptor])
    }
